...
<xsl:template name="xsl:initial-template">
  ...
  <xsl:variable
      name="fn"
      select="resolve-uri('../../build/website/@nineml/status.rss', static-base-uri())"/>

  <xsl:result-document href="{$fn}">
    <rss version="2.0">
      <channel>
        <title>@nineml</title>
        <link>https://nineml.org/@nineml/</link>
        <description>NineML status feed.</description>
  ...
  <xsl:for-each select="$years">
    ...
    <xsl:result-document
        href="{resolve-uri('../../build/website/@nineml/'||$year||'/index.html')}"
        method="html" html-version="5">
      <html>
        <head>
          <xsl:sequence select="$head-elements"/>
          <title>@nineml statuses in <xsl:sequence select="$year"/></title>
        </head>
        <body>
          <main>
            <article>
              <h1>@nineml statuses in <xsl:sequence select="$year"/></h1>
              <p>This page displays status
              updates
              about the NineML project from <xsl:sequence
              select="$year"/>.</p>
  ...
  <xsl:result-document href="{resolve-uri('../../build/website/@nineml/index.html')}"
                       method="html" html-version="5">
    <html>
      <head>
        <xsl:sequence select="$head-elements"/>
        <title>@nineml statuses</title>
      </head>
      <body>
        <main>
          <article>
            <h1>@nineml statuses</h1>
            <p>This page displays recent status
            updates
            about the NineML project.</p>
