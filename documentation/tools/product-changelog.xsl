...
<xsl:param name="nineml-product" as="xs:string" required="yes"/>
<xsl:param name="nineml-version" as="xs:string" required="yes"/>

<xsl:template match="/">
  <xsl:apply-templates select="/db:set/db:book/db:appendix[@xml:id='changelog']"/>
</xsl:template>

<xsl:template match="db:appendix">
  <xsl:variable name="revision" select="db:revhistory/db:revision[db:revnumber=$nineml-version]"/>
  <xsl:if test="count($revision) != 1">
    <xsl:message terminate="yes">Cannot find revision {$nineml-version} in changelog</xsl:message>
  </xsl:if>
...
<xsl:template match="db:revdescription">
  <body>
    <xsl:apply-templates select="*[not(@audience)]"/>
    <xsl:apply-templates select="*[@audience = $nineml-product]"/>
  </body>
</xsl:template>
